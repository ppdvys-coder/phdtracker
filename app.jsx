...
const ROLES = ["PhD","Researcher","Estate","Chula Lecturer","BSSC Lecturer","BSSC PGTA","Service/Admin","Personal","Unassigned"];
const ROLE_META = {
  "Unassigned":{th:"ยังไม่จัดหมวด", c:"#9E9E9E", period:"", start:""},
  "PhD":{th:"ปริญญาเอก", c:"#2B1241", period:"Sep 2023 – present", start:"2023-09-25"},
  "Researcher":{th:"นักวิจัย", c:"#6B4E8C", period:"", start:""},
...
const cols = {
  ...
  conferences: [
    {k:"name",l:"Conference",w:220},{k:"importance",l:"Importance",w:100,type:"select",opts:["High","Medium","Low"]},{k:"series",l:"Series / host",w:150,type:"tags"},{k:"url",l:"Website / CFP",w:220,type:"link"},
    {k:"targetYear",l:"Target year",w:90},{k:"date",l:"Event date",w:104},{k:"endDate",l:"Event end",w:104},{k:"location",l:"Location",w:150},
    {k:"topics",l:"Topics / themes",w:240,type:"tags"},
...
const seed = () => { const S = ({
  ...
  conferences: [
    { _a:[], name:"PRRES 2027 — 33rd Annual Conference + PhD Colloquium", importance:"High", series:"PRRES, annual, PLPR", url:"https://www.prres.org/conference", targetYear:"2027", date:"2027-01-17", endDate:"2027-01-20", location:"Curtin University, Perth, WA", topics:"Corporate real estate, Asset/property/facility management, IT & real estate, PropTech & big data, Green buildings & environmental policy, Property education, Built environment, PhD colloquium", abstractDue:"2026-09-30", paperDue:"2026-09-10", regDue:"2026-11-01", deadline:"2027-01-03", status:"Planning", lastChecked:"2026-08-28", notes:"33rd Annual PRRES Conference, Curtin University Perth, 17–20 Jan 2027 (incorporates 4th PLPR Regional Meeting). Theme: 'Shaping the property futures of education, research, the professions and policy'. Double-blind review; refereed papers published on prres.net. Abstracts & papers → conference@prres.org (early-bird abstracts close soon but late abstracts still accepted). KEY DATES: abstracts open 13 Jul 2026; early abstracts close 27 Jul; early-bird reg opens + abstract acceptance 3 Aug; FULL PAPER (refereed) 10 Sep 2026; refereed acceptance 14 Oct; early-bird reg closes 1 Nov; last day to register for paper inclusion (non-referee) 30 Nov; last day to register to attend 3 Jan 2027. PhD COLLOQUIUM (Sun 17 Jan): 3-page research summary + 150-word abstract + application form + supervisor declaration + enrolment evidence to Dr Wejen Reddy (wejendra.reddy@rmit.edu.au) by 30 Sep 2026; acceptance 16 Nov 2026; scholarship AUD1,500 (2nd–4th-yr FT property); Best Paper AUD400 (1st yr); Best Presenter AUD400. Summary format: Word A4, TNR 12, single-space, Harvard, filename surname_colloquium." },
    { _a:[], name:"AUDE27 — AUDE Annual Conference", importance:"Medium", series:"AUDE, annual", url:"https://www.aude.ac.uk/events/annual-conference/", targetYear:"2027", date:"2027-04-12", endDate:"2027-04-15", location:"University of Edinburgh", topics:"University estates, Higher-education estates, Campus, Facilities management, Directors of estates", abstractDue:"", paperDue:"", regDue:"", status:"Watching", lastChecked:"2026-08-28", notes:"Association of University Directors of Estates — very relevant to the smart-campus / university-estates PhD. Checked Aug 2026: dates & venue confirmed (‘Save the dates’), but REGISTRATION and CALL FOR PAPERS not yet released. Contact info@aude.ac.uk. Re-check for CFP + registration → then fill Abstract due / Register by." },
    { _a:[], name:"ICC 2027 — International Circularity Conference", importance:"Low", series:"Circularity, ICC", url:"https://www.circularity2027.com/", targetYear:"2027", date:"2027-02-07", endDate:"2027-02-13", location:"Murdoch University, Perth, WA", topics:"Circular business model/markets/policy, Circular construction & infrastructure, Circular mining & energy materials, Circular water/food/bioeconomy, Technology/data/innovation enablers", abstractDue:"2026-11-01", paperDue:"2027-02-28", regDue:"2026-11-30", status:"Watching", lastChecked:"2026-08-28", notes:"Shared by Michael. Relevant theme: ‘Circular construction & infrastructure’. Abstract submission opens 1 Aug 2026; abstract deadline 1 Nov 2026; acceptance 15 Nov 2026. Registration: Early Bird until 30 Nov 2026 · Standard until 1 Feb 2027 · Final after 2 Feb 2027. Full papers by 28 Feb 2027." },
  ],
  publications: [
...
function Dashboard({ m, data, update, setTab, resetAll, lang }) {
  ...
  const RORDER = ["PhD", "Researcher", "Estate", "Chula Lecturer", "BSSC Lecturer", "BSSC PGTA", "Service/Admin", "Personal", "Unassigned"];
...
function filterOutlookEvents(events, filter) {
  ...
  const hidePrivate = filter.hidePrivate !== false; // default on: Outlook masks Private-flagged events' titles as "Private Appointment" (real title not in the feed)
  return events.filter(e => {
    if (from && (e.date || "") < from) return false;
    if (hidePrivate && /^\s*(private appointment|private|busy)\s*$/i.test(e.summary || "")) return false; // masked/untitled — nothing useful to import
...
function AddHub({ data, setData, quickAdd, pushUndo, lang }) {
  ...
  const [showSession, setShowSession] = useState(false);
  const today = new Date().toISOString().slice(0, 10);
  const HATS = ["PhD", "Researcher", "Estate", "Chula Lecturer", "BSSC Lecturer", "BSSC PGTA", "Service/Admin", "Personal"];
  const items = [
    ...
    <div>
      ...
      <div style={{ marginTop: 16, background: OFF, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 16 }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", marginBottom: 10 }}>
          ...
          <label style={{ fontSize: 11, color: AUB2, display: "flex", alignItems: "center", gap: 5, cursor: "pointer" }} title={lang === "th" ? "Outlook ซ่อนชื่อ event ที่ตั้งเป็น Private เป็น 'Private Appointment' — ไม่มีข้อมูลให้ import" : "Outlook masks Private-flagged events as 'Private Appointment' — no real title to import"}><input type="checkbox" checked={outlookCfg.hidePrivate !== false} onChange={e => setOutlookCfg({ hidePrivate: e.target.checked })} />{lang === "th" ? "ข้าม Private Appointment" : "Skip Private Appointments"}</label>
        ...
        <div style={{ background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 8, padding: "10px 12px", marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: AUB2 }}>{lang === "th" ? "แยกอัตโนมัติ (คำในหัวข้อ → หมวก + หมวด)" : "Auto-assign (title keyword → hat + category)"}</span>
            {(!outlookCfg.rules || !outlookCfg.rules.length) && <button onClick={() => setOutlookCfg({ rules: [{ match: "dibam, bssc, tutorial, dissertation", hat: "BSSC PGTA", cat: "Teaching – delivery" }, { match: "seminar, webinar, lecture series, talk, symposium, spring school", hat: "PhD", cat: "Seminar" }, { match: "social, networking, drinks, mixer, welcome, christmas, party", hat: "Personal", cat: "Networking" }, { match: "estate, estates, iwms, mycampus, huddle, facilities, cafm, space, building", hat: "Estate", cat: "Meeting" }, { match: "phd, readiness, supervis, interview, board, catch, governance, transformation", hat: "PhD", cat: "Meeting" }], fallback: "Unassigned" })} style={{ border: `1px solid ${AUB}`, background: CARD, color: AUB, borderRadius: 5, padding: "3px 9px", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>{lang === "th" ? "＋ ใส่กฎแนะนำ (PhD/Lecturer)" : "＋ Use suggested rules"}</button>}
          </div>
          ...
          {(outlookCfg.rules || []).map((r, i) => (
            <div key={i} style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 6, flexWrap: "wrap" }}>
              ...
              <select value={r.hat || "PhD"} onChange={e => { const rules = (outlookCfg.rules || []).slice(); rules[i] = { ...rules[i], hat: e.target.value }; setOutlookCfg({ rules }); }} title={lang === "th" ? "หมวก (บทบาท)" : "hat (role)"} style={{ border: `1px solid ${BORDER}`, borderRadius: 5, padding: "4px 7px", fontSize: 11, cursor: "pointer" }}>{ROLES.map(h => <option key={h} value={h}>{roleLab(lang, h)}</option>)}</select>
              <span style={{ color: GREY, fontSize: 10 }}>+</span>
              <select value={r.cat || "Meeting"} onChange={e => { const rules = (outlookCfg.rules || []).slice(); rules[i] = { ...rules[i], cat: e.target.value }; setOutlookCfg({ rules }); }} title={lang === "th" ? "หมวดกิจกรรม" : "activity category"} style={{ border: `1px solid ${ACT_COLOR[r.cat || "Meeting"] || BORDER}`, color: "#fff", background: ACT_COLOR[r.cat || "Meeting"] || GREY, fontWeight: 700, borderRadius: 5, padding: "4px 7px", fontSize: 11, cursor: "pointer" }}>{ACT_CATS.map(c => <option key={c} value={c} style={{ color: INK, background: "#fff", fontWeight: 400 }}>{c}</option>)}</select>
          ...
          {(outlookCfg.rules || []).length > 0 && (
            <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginTop: 4 }}>
              <button onClick={() => setOutlookCfg({ rules: [...(outlookCfg.rules || []), { match: "", hat: "PhD", cat: "Meeting" }] })} style={{ border: `1px solid ${BORDER}`, background: "#fff", color: AUB2, borderRadius: 5, padding: "3px 9px", cursor: "pointer", fontSize: 11 }}>{lang === "th" ? "＋ เพิ่มกฎ" : "＋ Add rule"}</button>
            ...
            <div style={{ padding: "2px 8px", overflowY: "auto", flex: 1 }}>
              {review.map((it, idx) => (
                <div key={idx} style={{ display: "flex", gap: 8, alignItems: "center", padding: "7px 6px", borderBottom: `1px solid ${BORDER}`, fontSize: 12, opacity: it.keep ? 1 : 0.45 }}>
                  ...
                  <select value={it.role} onChange={e => setReviewItem(idx, { role: e.target.value })} disabled={!it.keep} title={lang === "th" ? "หมวก (บทบาท)" : "hat (role)"} style={{ border: `1px solid ${BORDER}`, borderRadius: 5, padding: "3px 6px", fontSize: 11, cursor: "pointer", flex: "0 0 auto" }}>{ROLES.map(rr => <option key={rr} value={rr}>{roleLab(lang, rr)}</option>)}</select>
                  <select value={it.cat || "Meeting"} onChange={e => setReviewItem(idx, { cat: e.target.value })} disabled={!it.keep} title={lang === "th" ? "หมวดกิจกรรม" : "activity category"} style={{ border: `1px solid ${it.keep ? (ACT_COLOR[it.cat || "Meeting"] || BORDER) : BORDER}`, color: it.keep ? "#fff" : GREY, background: it.keep ? (ACT_COLOR[it.cat || "Meeting"] || GREY) : "#fff", fontWeight: 700, borderRadius: 5, padding: "3px 6px", fontSize: 11, cursor: "pointer", flex: "0 0 auto" }}>{ACT_CATS.map(c => <option key={c} value={c} style={{ color: INK, background: "#fff", fontWeight: 400 }}>{c}</option>)}</select>
                </div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 8, padding: "12px 16px", borderTop: `1px solid ${BORDER}` }}>
